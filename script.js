document.addEventListener('DOMContentLoaded', () => {
    ...
    // Variáveis do Vídeo Holográfico
    ...
    function speakText(text, onStartCallback, onEndCallback) {
        if ('speechSynthesis' in window) {
            ...
            const femaleVoice = synthVoices.find(v => 
                v.name.includes('Francisca') || v.name.includes('Luciana') || 
                v.name.includes('Google português do Brasil') || v.name.includes('Zira') || 
                v.name.includes('Vitoria') || v.name.includes('Helena')
            );
            ...
            utterance.onstart = () => { 
                // AQUI A MÁGICA ACONTECE: O vídeo dá o play junto com a fala!
                if(aiVideo) aiVideo.play().catch(e => console.log("Erro de autoplay no vídeo"));
                if(onStartCallback) onStartCallback(); 
            };
            
            utterance.onend = () => { 
                // A fala termina, o vídeo pausa e reseta
                if(aiVideo) { aiVideo.pause(); aiVideo.currentTime = 0; }
    ...
    async function fetchSystemData() { 
        ...
        } catch (e) { console.error("Falha ao sincronizar Planilha DIC.", e); } 
    }
    ...
    function createSuccessCard(data) {
        ...
        let htmlContent = `
          ...
          <div class="card-body">
            ...
            <div class="data-row"><div class="data-label" style="align-self: flex-start;">HISTÓRICO:</div><div class="data-value" style="line-height: 1.4;">${historicoHtml}</div></div>
            <div class="data-row"><div class="data-label">AVALIAÇÃO:</div><div class="data-value">${statusPolicialText}</div></div>
            <div class="groups-container"><div class="groups-title">${groupTitle} (${groupsToUse.length})</div>
        `;
        ...
        typeWriterEffect(card.querySelector('.matrix-title'), data.nick);
    }
    ...
    function createErrorCard(nick, isConnectionFail = false) {
        ...
        resultsGrid.appendChild(card); 
        typeWriterEffect(card.querySelector('.matrix-title'), nick);
    }
    ...
    btnClearFilters.addEventListener('click', () => {
        chkAllGroups.checked = false; chkOtherPolice.checked = false; chkOffline.checked = false; chkHidden.checked = false;
    ...
    btnSearch.addEventListener('click', async () => {
        ...
        for (let i = 0; i < nicks.length; i += CHUNK_SIZE) {
            ...
            scanStatus.textContent = `Varredura rápida: Lote ${Math.floor(i / CHUNK_SIZE) + 1} de ${totalChunks}...`; 
            playSound('scan');
        ...
        const totalRiscos = updateAnalyticsHUD(); 
        analyticsPanel.style.display = "flex"; 
        btnSearch.disabled = false;
    
        // ===============================================
        // MÁGICA FINAL: IA APARECE COM O SEU VÍDEO
        // ===============================================
        window.scrollTo({ top: 0, behavior: 'smooth' }); 
        
        if (aiOverlay) {
            setTimeout(() => {
                aiOverlay.classList.add('active'); // Escurece a tela e mostra o vídeo
                ...
                setTimeout(() => {
                    ...
                    speakText(msg, null, () => { 
                        setTimeout(() => { 
                            aiOverlay.classList.remove('active'); // Oculta o vídeo ao final
                        }, 500); 
                    });
                }, 1000); 
            }, 800); 
